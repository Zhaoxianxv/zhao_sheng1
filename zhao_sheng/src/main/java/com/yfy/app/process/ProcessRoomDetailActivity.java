...
public class ProcessRoomDetailActivity extends BaseActivity implements Callback<ResEnv> {
    private static final String TAG = ProcessRoomDetailActivity.class.getSimpleName();
...
//        }
        ...
        for (ProLayerOneData oneData:info.getRecordvalue()){
            KeyValue value=new KeyValue(StringUtils.getTextJoint("%1$s:\t", oneData.getTitle()), StringUtils.getTextJoint("%1$s\t%2$s", oneData.getDisplaycontent(), oneData.getUnit()));
            value.setType(oneData.getValuetype());
            switch (oneData.getValuetype()){
                case "image":
                    value.setListValue(StringUtils.listToStringSplitCharacters(oneData.getContent(),","));
                    break;
            }
